...
- Docker (image `opendronemap/odm:3.5.4` bakal ke-pull otomatis pas pertama
  jalan).
- GDAL (`gdal_calc.py` di PATH) — dipakai buat hitung NDVI, jalan di host
  langsung (bukan di dalam container ODM).
...
- `POST /projects/{name}/process` — trigger pipeline. Panggil ini SETELAH
  ...
  server. Balas `202` begitu diterima; extract (split RGB/MS) + ODM + NDVI
  jalan di background.
...
- `ms_orthomosaic.tif`, `ndvi.tif` — cuma ada kalau zip-nya ngandung foto
  multispektral (`*_MS_G.TIF` dkk).
